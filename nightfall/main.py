"""FastAPI application: clean public API -> translated signed upstream calls."""
...
from .translate import (normalize_downloads, normalize_search,
                        normalize_streams, normalize_subtitles)
...
async def lifespan(app: FastAPI):
    ...
    state["identity"] = DeviceIdentity(cfg.device_file, state["store"].data,
...
app = FastAPI(title="🌙 Nightfall — MovieBox Gateway (anime separated)",
              description="MovieBox-only gateway (anime separated to ../anime-app). Clean API that translates requests into official "
                          "MovieBox app traffic, with self-healing when the "
                          "app rotates its protocol secrets.",
...
def _err(status: int, code: str, message: str, **extra) -> JSONResponse:
    ...
    if state.get("detector") and \
            state["detector"].status()["state"] == "PROTOCOL_STALE":
        resp.headers["X-Wrapper-State"] = "PROTOCOL_STALE"
        resp.headers["X-Wrapper-Remediation"] = (
            "drop latest APK into wrapper/watch/ then POST /heal")
    return resp
...
def health():
    ...
    return {
        ...
        "consecutive_auth_failures": det["consecutive_auth_failures"],
        "protocol_version": state["store"].data.get("version"),
...
def doctor(live: bool = False):
    rep = doctor_run(state["store"], state["detector"], live=live,
                     client_factory=lambda s, i: UpstreamClient(s, i))
...
def set_session_token(body: dict = Body(...)):
    """Plug a captured session token (from your own logged-in device) into protocol.yaml."""
    token = str(body.get("token") or "").strip()
    if not token:
        return _err(400, "BAD_REQUEST", "JSON body must be {\"token\": \"...\"}")
    data = dict(state["store"].data)
    data["session_token"] = token
    state["store"].save(data, backup_dir=settings().backups_dir)
    state["client"] = build_client()
    return {"ok": True, "message": "session token stored; subject-api endpoints "
                                   "will now send Authorization",
...
@app.get("/protocol")
def protocol_view():
    return mask_secrets(state["store"].data)


@app.get("/keys")
def keys_list():
    return {"ok": True, "keys": state["keys"].list()}
...
def keys_create(body: dict = Body(...)):
    ...
    return {"ok": True, "message": "store this key now - it is shown once",
            **created}
...
def search(q: str = Query(min_length=1),
    ...
    try:
        res = _call("search", f"search:{q}:{page}:{per_page}:{type}",
                    lambda: state["client"].request(ep, json_body=body), debug_raw)
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
    except UpstreamNetworkError as exc:
        return _err(504, "UPSTREAM_UNREACHABLE", str(exc))
...
def search_rank():
    ...
    try:
        return _call("metadata", "rank", lambda: state["client"].request(ep))
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
...
def title_detail(subject_id: str, debug_raw: bool = False):
    ...
    try:
        res = _call("metadata", f"detail:{subject_id}",
                    lambda: state["client"].request(ep, params={"subjectId": subject_id}),
                    debug_raw)
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
    return res
...
def episodes(subject_id: str, season: int = 1, start: int = 1, end: int = 100):
    ...
    try:
        return _call("metadata", f"eps:{subject_id}:{season}:{start}:{end}",
                     lambda: state["client"].request(ep, params=params))
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
...
async def stream(request: Request, subject_id: str,
                 ...
                 mode: Optional[str] = None, index: int = 0,
                 debug_raw: bool = False):
    ...
    try:
        res = _call("stream", f"stream:{subject_id}:{s}:{e}",
                    lambda: state["client"].request(ep_def, params=params), debug_raw)
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
    ...
    # If explicit_se was not given and 0/0 yielded nothing, fallback to 1/1 (or vice versa)
    if not streams and explicit_se is None and explicit_ep is None:
        try:
            res = _call("stream", f"stream:{subject_id}:1:1",
                        lambda: state["client"].request(ep_def, params={"subjectId": subject_id, "se": 1, "ep": 1}), debug_raw)
            streams = normalize_streams(res.get("data"))
        except Exception:
            pass

    if not streams:
        return _err(404, "NO_STREAMS_FOUND",
                    "no URLs found in play-info payload; inspect with ?debug_raw=true")
    if resolution:
        want = resolution.rstrip("pP")
        exact = [st for st in streams
                 if want in {str(r) for r in st.get("resolutions") or []}
                 or str(st.get("max_resolution", "")).rstrip("pP") == want]
        if exact:
            streams = exact
    chosen = streams[min(index, len(streams) - 1)]
    effective_mode = mode or settings().get("mode", "links")
    if effective_mode == "proxy":
        return await relay(chosen["url"], request)
    return {"ok": True, "selected": chosen,
            "alternatives": [{"kind": st["kind"], "resolutions": st.get("resolutions"),
...
async def download(request: Request, subject_id: str,
                   ...
                   resolution: int = 0,
                   mode: Optional[str] = None, index: int = 0,
                   min_resolution: int = 0, debug_raw: bool = False):
    """Direct download resources.

    season/episode (or se/ep aliases) filter to one episode when both given.
    Aggregates `pages` upstream pages so the episode catalog is complete.
    """
    ...
    for p in range(page, page + max(1, pages)):
        params = {"subjectId": subject_id, "page": p, "perPage": per_page,
                  "all": 0, "startPosition": 1, "endPosition": per_page,
                  "pagerMode": 0, "resolution": resolution}
        if s is not None:
            params["se"] = s
            params["epFrom"] = e if e is not None else 1
            params["epTo"] = e if e is not None else per_page * p or per_page
        try:
            res = state["client"].request(ep_def, params=params)
        except UpstreamAuthError as exc:
            return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
        data = res.get("data")
    ...
    # upstream ignores se/ep on this endpoint -> filter client-side
    if s is not None and e is not None:
        items = [it for it in items
                 if str(it.get("se")) == str(s) and str(it.get("ep")) == str(e)]

    if debug_raw:
        return {"ok": True, "upstream_code": res.get("upstream_code"),
                "count": len(items), "list": items}

    # normalize with full metadata preserved
    resources = []
    for it in items:
        ...
        if not url:
            continue
        try:
            size_mb = int(it.get("size") or 0) / 1e6
        except (TypeError, ValueError):
            size_mb = None
        resources.append({
            "se": it.get("se"), "ep": it.get("ep"),
            "episode": it.get("episode"),
            "title": it.get("title"),
            "resolution": f"{it.get('resolution')}p" if it.get("resolution") else None,
            "codec": it.get("codecName"),
            "size_mb": round(size_mb, 1) if size_mb else None,
            "duration_seconds": it.get("duration"),
            "require_member_type": it.get("requireMemberType"),
            "signed_url": url,
            "source_url": it.get("sourceUrl"),
        })
    # Sort resources highest resolution first (1080p > 720p > 480p > 360p)
    resources.sort(key=lambda r: _res_num(r.get("resolution")), reverse=True)
    if min_resolution:
        resources = [r for r in resources
                     if _res_num(r["resolution"]) >= min_resolution] or resources
    if not resources:
        return _err(404, "NO_DOWNLOADS_FOUND",
                    "no resource links; try ?debug_raw=true to inspect payload")

    chosen = resources[min(index, len(resources) - 1)]
    effective_mode = mode or settings().get("mode", "links")
    if effective_mode == "proxy":
        return await relay(chosen["signed_url"], request)
    return {"ok": True, "selected": chosen, "count": len(resources),
            "all": resources}


def _res_num(resolution) -> int:
    try:
        return int(str(resolution).rstrip("pP"))
    except (TypeError, ValueError):
        return 0


@app.get("/titles/{subject_id}/resources")
async def resources_alias(subject_id: str, request: Request,
                          season: Optional[int] = None, episode: Optional[int] = None,
                          se: Optional[int] = None, ep: Optional[int] = None,
                          page: int = 1, pages: int = 3, per_page: int = 10,
                          resolution: int = 0, index: int = 0, debug_raw: bool = False):
    """Explicit alias of /download for clarity."""
    return await download(request, subject_id, season=season, episode=episode,
                          se=se, ep=ep, page=page, pages=pages,
                          per_page=per_page, resolution=resolution,
                          index=index, debug_raw=debug_raw)
...
def subtitles(subject_id: str, season: int = 1, episode: int = 1):
    ...
    try:
        ...
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
    return {"ok": True, "subtitles": normalize_subtitles(res.get("data")),
            "upstream_code": res.get("upstream_code")}
...
def discover_tabs(tab_id: int = 1, version: str = "0"):
    ...
    try:
        return _call("metadata", f"tabs:{tab_id}:{version}",
                     lambda: state["client"].request(ep, params={"tabId": tab_id,
                                                                 "version": version}))
    except UpstreamAuthError as exc:
        return _err(502, "UPSTREAM_AUTH_REJECTED", str(exc))
...
async def proxy_media(request: Request, url: str):
    """Byte-relay for media URLs (Range supported). Allowlist enforced."""
